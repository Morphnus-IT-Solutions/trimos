...
class DocType:
  ...
  def get_comp(self):
    ...
    if has_common(self.roles, ['CRM Manager', 'Sales Manager', 'System Manager']):
      type.append('Sales')
    if has_common(self.roles, ['Purchase Manager', 'System Manager']):
...
#    return ret
  ...
  # Gets Transactions type and Group By options based on module (Overwritten)
  #---------------------------------------------------------------------------
  def get_trans_group(self,module):
    ret = {}
    st = ''
    stmt,st_type,group,group_by = [],[],[],[]
    if module == 'Sales':
      stmt = ['Quotation','Sales Order','Delivery Note']
      group = ['Item','Item Group','Item Type','Brand','Customer','Customer Group','Territory','Zone','Sales Person']
    elif module == 'Service':
      stmt = ['Service Quotation','Service Order']
      group = ['Serial No','Item','Item Group','Item Type','Brand','Customer','Customer Group','Territory','Zone','Sales Person']
