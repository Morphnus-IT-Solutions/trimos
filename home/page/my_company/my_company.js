...
MemberProfile = function(parent, uid, member_item) {
	...
	this.make_toolbar = function() {
		if(has_common(['Administrator','System Manager'],user_roles)) {
			...
			var pwd_btn = $btn(this.toolbar_area, 'Update Password', function(){ me.update_password(); }, {marginRight:'3px'});
		}
	}
	...
	this.update_password = function(){
		...
		d.fields_dict.change.input.onclick = function() {
			...
			if(v) {
				...
				this.set_working();
				$c_page('home','my_company','change_password', JSON.stringify({'new_password':v.new_password1,'user_id':me.uid}),function(r,rt) {
