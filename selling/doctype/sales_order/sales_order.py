...
# ===============================================================================================
	...
	def update_lead_status(self, status):
    		if self.doc.inquiry_no:
      			sql("update tabLead set status = '%s', modified = '%s' where name = '%s'" % (status, nowdate(), self.doc.inquiry_no))
	...
	def on_submit(self):
		...
		set(self.doc, 'status', 'Submitted')
    		...
    		# update lead status
   		self.update_lead_status('Order Confirmed')
...
# ===============================================================================================
	def on_cancel(self):
		...
		# update lead status
    		status = self.doc.quotation_to and 'Quotation Given' or 'Open'
   	 	self.update_lead_status(status)
