...
cur_frm.cscript.onload = function(doc, cdt, cdn) {
  ...
  if(doc.__islocal){
    hide_field(['customer_address','contact_person','customer_name','address_display','contact_display','contact_mobile','contact_email','territory','customer_group','shipping_address', 'zone', 'contact_department', 'contact_designation']);
  }
}
...
cur_frm.cscript.refresh = function(doc, cdt, cdn) {
  ...
  if(doc.customer) unhide_field(['customer_address','contact_person','customer_name','address_display','contact_display','contact_mobile','contact_email','contact_department','contact_designation','territory','zone','customer_group','shipping_address']);
...
// -------------------------------------
// pull quotation details (overwritten)
// -------------------------------------
cur_frm.cscript['Pull Quotation Details'] = function(doc,dt,dn) {
  var callback = function(r,rt){
    var doc = locals[cur_frm.doctype][cur_frm.docname];
    if(r.message){
      doc.quotation_no = r.message;
                  if(doc.quotation_no) {
                          unhide_field(['quotation_date','customer_address','contact_person','customer_name','address_display','contact_display','contact_mobile','contact_email','territory','zone','customer_group','shipping_address', 'contact_department', 'contact_designation']);
                          if(doc.customer) get_server_fields('get_shipping_address',doc.customer,'',doc, dt, dn, 0);
                  }
                  cur_frm.refresh();
    }
...
*/
// ------------------------------------
// pull project customer (overwritten)
// ------------------------------------
cur_frm.cscript.project_name = function(doc,cdt,cdn){
  $c_obj(make_doclist(doc.doctype, doc.name),'pull_project_customer','', function(r,rt){
    refresh_many(['customer','customer_name', 'customer_address', 'contact_person', 'territory','zone','contact_mobile','contact_email', 'contact_department', 'contact_designation', 'customer_group']);
  });
}
...
cur_frm.cscript.sub_category = function(doc, cdt, cdn) {
  if(doc.sub_category == 'DI') {
    unhide_field(['supplier_name', 'order_acknowledgement', 'document_no', 'document_date', 'order_no'])
  } else {
    hide_field(['supplier_name', 'order_acknowledgement', 'document_no', 'document_date', 'order_no'])
  }
}
// --------------------
// get inquiry details
