...
class DocType(TransactionBase):
	...
	def get_contact_details(self, obj = '', primary = 0):
		...
		contact = sql("select contact_name, contact_no, email_id, contact_address, department, designation from `tabContact` where customer = '%s' and docstatus != 2 %s" %(obj.doc.customer, cond), as_dict = 1)
		if not contact:
			return
		c = contact[0]
		...
		obj.doc.department = c['department'] or ''
		obj.doc.designation = c['designation'] or ''
	...
	def get_customer_details(self, obj = '', inv_det_reqd = 1):
		details = sql("select customer_name,address, territory, customer_group, default_sales_partner, default_commission_rate, zone from `tabCustomer` where name = '%s' and docstatus != 2" %(obj.doc.customer), as_dict = 1)
		...
		obj.doc.zone = details and details[0]['zone'] or ''
	...
	def get_item_details(self, item_code, obj):
		if not obj.doc.price_list_name:
			...
			raise Exception
		item = sql("select description, item_name, brand, item_group, stock_uom, default_warehouse, default_income_account, default_sales_cost_center, description_html, product_code, item_type from `tabItem` where name = '%s' and (ifnull(end_of_life,'')='' or end_of_life >	now() or end_of_life = '0000-00-00') and (is_sales_item = 'Yes' or is_service_item = 'Yes')" %(item_code), as_dict=1)
	...
	def get_tc_details(self,obj):
		...
		idx = 0
		obj.doc.clear_table(obj.doclist,'tc_details')
...
#---------------------------------------- Get Tax Details -------------------------------#
	...
	def get_serial_details(self, serial_no, obj):
		item = sql("select item_code, make, label, brand, description, product_code, item_type from `tabSerial No` where name = '%s' and docstatus != 2" %(serial_no), as_dict=1)
		...
		ret = {
			...
			'product_code'    			: item and item[0]['product_code'] or '',  # custom
			'item_type'				: item and item[0]['item_type'] or '',     # custom
			'item_tax_rate'				: str(t)
		}
		return ret
...
# ============================== custom server code ================================== #
  ...
  # ------------------
	def get_lead_details(self,obj):
		inquiry = sql("select `tabLead`.date, `tabLead`.by_whom from `tabLead` where `tabLead`.name = '%s'"%(obj.doc.inquiry_no), as_dict =1)  
		ret = {
      'inquiry_date'           :   inquiry and inquiry[0]['date'].strftime('%Y-%m-%d') or '',
      'inquiry_generated_by'   :   inquiry and inquiry[0]['by_whom'] or ''
		}
		return str(ret)


  # to calculate gross profit (overwritten)
  # ----------------------------------------
	def calc_gross_profit(self, obj):
		gross_profit = 0
		cost_rate = 0
		sell_rate = 0
		for d in getlist(obj.doclist,obj.fname):
			purchase_rate = sql("select purchase_rate from `tabItem` where name = '%s'" %(d.item_code), as_dict = 1)
			purchase_rate = purchase_rate and purchase_rate[0]['purchase_rate'] or 0
			if purchase_rate:
				gross_profit += ((flt(d.basic_rate) - (flt(purchase_rate))) * flt(d.qty))
				sell_rate += (flt(d.basic_rate) * flt(d.qty))
				cost_rate += (flt(purchase_rate) * flt(d.qty))
			obj.doc.gross_profit = flt(gross_profit)
			if cost_rate:
				obj.doc.gross_profit_percent = (flt(sell_rate) - flt(cost_rate))  * 100 / flt(cost_rate)

  #add to Calendar (for Monthly Visit Schedule, Local Travel, Outstation Travel and Tour Advance)
  # -----------------------------------------------------------------------------------------------
...
#------------------------------------- Approval Process ----------------------------------#
  ...
  # ----------------------
	def send_for_approval(self,territory,message,sub,doctype,name):
		...
		for d in send:
			...
			'''
			msg = message+" "+login
		sendmail(['rupali_s@trimosmetrology.net', 'ajay_d@trimosmetrology.net'], sender = 'crm@trimosmetrology.net', subject=sub, parts=[['text/plain', msg]])
  ...
  # -------------------------
	def send_feedback(self,name,message,sub):
		...
		msg = message
		sendmail(send, sender='crm@trimosmetrology.net', subject=sub, parts=[['text/plain', msg]])
