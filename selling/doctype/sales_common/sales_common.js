...
// ******************** ITEM CODE ********************************
// added custom fields brand and product_code to be available in search
cur_frm.fields_dict[cur_frm.cscript.fname].grid.get_field("item_code").get_query = function(doc, cdt, cdn) {
  if (doc.order_type == 'Maintenance')
    return 'SELECT tabItem.name,tabItem.item_name,tabItem.description,tabItem.brand,tabItem.product_code FROM tabItem WHERE tabItem.is_service_item="Yes" AND tabItem.docstatus != 2 AND (ifnull(`tabItem`.`end_of_life`,"") = "" OR `tabItem`.`end_of_life` > NOW() OR `tabItem`.`end_of_life`="0000-00-00") AND tabItem.%(key)s LIKE "%s" LIMIT 50';
  else 
    return 'SELECT tabItem.name,tabItem.item_name,tabItem.description,tabItem.brand,tabItem.product_code FROM tabItem WHERE tabItem.is_sales_item="Yes" AND tabItem.docstatus != 2 AND (ifnull(`tabItem`.`end_of_life`,"") = "" OR `tabItem`.`end_of_life` > NOW() OR `tabItem`.`end_of_life`="0000-00-00") AND tabItem.%(key)s LIKE "%s" LIMIT 50';
}
...
// ************************ BASE REF RATE ****************************
cur_frm.cscript.base_ref_rate = function(doc, cdt, cdn){
  var d = locals[cdt][cdn];
  set_multiple(cur_frm.cscript.tname, d.name, {'basic_rate': flt(d.base_ref_rate) * (100 - flt(d.adj_rate)) / 100}, cur_frm.cscript.fname);
...
cur_frm.cscript.update_fname_table = function(doc , tname , fname , n) {
  ...
  for(var i=0;i<cl.length;i++) {
    if(n == 1){
      ...
      set_multiple(tname, cl[i].name, {'base_ref_rate': flt(flt(cl[i].ref_rate) * flt(doc.conversion_rate))}, fname);

      if(cl[i].ref_rate > 0)
        set_multiple(tname, cl[i].name, {'adj_rate': 100 - flt(flt(cl[i].export_rate) * 100 / flt(cl[i].ref_rate))}, fname);
    }
    else if(n == 4){
      set_multiple(tname, cl[i].name, {'ref_rate': flt(flt(cl[i].base_ref_rate) / flt(doc.conversion_rate))}, fname);
      set_multiple(tname, cl[i].name, {'export_rate': flt(flt(cl[i].basic_rate) / flt(doc.conversion_rate))}, fname);
      set_multiple(tname, cl[i].name, {'amount' : flt(flt(cl[i].basic_rate) * flt(cl[i].qty)), 'export_amount': flt(flt(cl[i].export_rate) * flt(cl[i].qty))}, fname);
    }
    net_total += flt(flt(cl[i].qty) * flt(cl[i].basic_rate));
  }
...
// -----------------------
cur_frm.add_fetch('factor_figure', 'factor_figure_rate', 'conversion_rate');

cur_frm.add_fetch('sales_person', 'designation', 'sales_designation');


cur_frm.cscript.incentive_percent = function(doc, cdt, cdn) {
  var fname = cur_frm.cscript.sales_team_fname;
  var d = locals[cdt][cdn];
  if (d.incentive_percent) {
    d.incentive_amount = flt(flt(doc.net_total)*flt(d.incentive_percent)/100);
    refresh_field('incentive_amount', d.name, fname);
  }
}

cur_frm.cscript.custom_recalc = function(doc, cdt, cdn) {
  var sales_team = cur_frm.cscript.sales_team_fname;
  cl = getchildren('Sales Team', doc.name, sales_team);
  for(var i=0;i<cl.length;i++) {
    if (cl[i].incentive_percent) {
      cl[i].incentive_amount = flt(flt(doc.net_total)*flt(cl[i].incentive_percent)/100);
      refresh_field('incentive_amount', cl[i].name, sales_team);
    }
  }
}

/* ============================= custom client script end ================================== */
