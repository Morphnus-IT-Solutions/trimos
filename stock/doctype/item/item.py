...
class DocType:
	...
	def overwrite(self):
		...
		for d in getlist(self.doclist,'ref_rate_details'):
			exists = check_dict.get(cstr(d.price_list_name)+'~~~'+cstr(d.ref_currency), '')
			if exists:
				exists.append(d.name)
			else:
				check_dict[cstr(d.price_list_name)+'~~~'+cstr(d.ref_currency)] = [d.name]
		valid_values = [] # ['price_list_name', 'ref_rate', 'name']
		for c in check_dict:
			val = check_dict[c]
			val = sorted(val)
			if len(val) > 1:
				valid_values.append([c.split('~~~')[0], c.split('~~~')[1], val[-1]])
			else:
				valid_values.append([c.split('~~~')[0], c.split('~~~')[1], val[0]])
		for v in valid_values:
			sql("delete from `tabRef Rate Detail` where price_list_name = '%s' and ref_currency = '%s' and parent = '%s' and name != '%s'" % (v[0], v[1], self.doc.name, v[2]))
