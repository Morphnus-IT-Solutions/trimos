...
class DocType(TransactionBase):
	def __init__(self, doc, doclist=[]):
        ...
        def get_delivery_address(self):
                details = sql("select name, address_line1, address_line2, city, country, pincode, state, phone from `tabAddress` where customer = '%s' and docstatus != 2 order by is_primary_address desc limit 1" % self.doc.customer, as_dict = 1)
                ...
                self.doc.delivery_address = address_display
